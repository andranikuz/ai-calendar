...
1. **Инициализация Go модуля** - go mod init
2. **Создание структуры каталогов** согласно Clean Architecture
3. **Настройка базовых зависимостей** (gin, pgx, redis)
4. **Docker окружение** для локальной разработки
5. **Первые domain entities** и бизнес-логика